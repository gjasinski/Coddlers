...
import pl.coddlers.git.models.ResponseWithPathWithNamespaceDto;
...
public class GitLessonService {
    ...
    @Value("${pl.coddlers.git.host}:${pl.coddlers.git.port}")
    private String gitLabHostAndPort;
    ...
    public CompletableFuture<String> getRepositoryUrl(Long gitRepositoryId) {
        return CompletableFuture.supplyAsync(getRepositoryUrlSupplier(gitRepositoryId), executor);
    }

    private Supplier<String> getRepositoryUrlSupplier(Long repositoryId) {
        return () -> {
            String resourceUrl = gitlabApiProjects + repositoryId;
            HttpHeaders headers = getHttpHeaders();

            UriComponentsBuilder builder = createComponentBuilder(resourceUrl);
            HttpEntity<?> entity = new HttpEntity<>(headers);

            ResponseEntity<ResponseWithPathWithNamespaceDto> exchange = restTemplate.exchange(
                    builder.build().toUriString(),
                    HttpMethod.GET,
                    entity,
                    ResponseWithPathWithNamespaceDto.class);
            return exchange.getBody().getPathWithNamespace();
        };
    }
}
